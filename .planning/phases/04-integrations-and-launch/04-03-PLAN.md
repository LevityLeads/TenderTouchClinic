...
must_haves:
  truths:
    ...
    - "Site components meet automated accessibility standards"
...
**Step 2: Test Booking Widget on Mobile (BOOK-03)**
1. Open Chrome DevTools (F12 or Ctrl+Shift+I)
2. Toggle device toolbar (Ctrl+Shift+M)
3. Select "iPhone SE" (375px width) or "Pixel 5" (393px width)
4. Navigate to /book:
   - Verify no horizontal scrollbar appears
   - Verify widget container fits within viewport
   - Verify fallback links/buttons are at least 44x44px (right-click > inspect to check)
5. Test touch interactions by clicking links (they should be easy to tap)

**Step 3: Test WhatsApp Disclaimer**
1. Hover over WhatsApp button (bottom-right)
2. Verify tooltip shows "General inquiries only" or similar disclaimer
3. Click button - verify WhatsApp opens with pre-filled message
4. Verify tooltip is CSS-only (no JavaScript flickering/delay)
...
4. Mobile verification (BOOK-03): Chrome DevTools mobile emulation confirms no overflow, adequate touch targets
5. PageSpeed Insights shows LCP < 2.5s, CLS < 0.1 (or document actual values)
6. Keyboard navigation works throughout site
</verification>
...
- Mobile booking widget verified working (no overflow, 44px+ touch targets)
