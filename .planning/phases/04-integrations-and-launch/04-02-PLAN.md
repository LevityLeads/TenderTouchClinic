...
Output: Updated WhatsApp button with tooltip, cookie consent banner component wired into layout.
</objective>
...
<task type="auto">
  <name>Task 1: Update WhatsApp button with non-clinical disclaimer</name>
  <files>
    src/components/layout/whatsapp-button.tsx
  </files>
  <action>
Update WhatsApp button to include WHATS-03 disclaimer:

1. Convert to Client Component ('use client') to support hover/focus tooltip
2. Add Tooltip component (using CSS-only approach or existing Radix UI Tooltip if available)
3. Show tooltip on hover/focus with text: "For general inquiries only - not for medical advice"
4. Update aria-label to include disclaimer: "Chat with us on WhatsApp for general inquiries"
5. Keep existing styling and positioning (fixed bottom-6 right-6 z-40)
6. Maintain 56x56px touch target (w-14 h-14)

Tooltip implementation (CSS-only for simplicity):
- Use group class on container
- Position tooltip above button with arrow
- Show on hover/focus with opacity transition
- Text: "For directions, hours, and general questions"
- Secondary line: "Not for medical advice"

Alternative approach if tooltip too complex:
- Add small text label below button that's always visible on larger screens
- Text: "General inquiries only" in small text
- Hide on mobile (space constraint) but show in tooltip on hover
  </action>
  <verify>
...
- Button still functions (opens WhatsApp link)
  ...
  <done>
WhatsApp button updated with non-clinical inquiry disclaimer visible on hover/focus.
  </done>
</task>
...
2. Hover over WhatsApp button - disclaimer tooltip visible
